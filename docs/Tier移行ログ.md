...
- A-2b（実 apply 連携・記録）完了：実行結果と実行ログ参照を記録・監査し、read-only の実 AWS 接続を最小権限の専用資格で確認（apply は人の承認ゲート・バックエンドは apply を実行しない）。

## 2026-07-09: 公開デプロイ（Tier 4 で常時稼働を開始）

実装と read-only 連携が済んだので、AWS 上で常時稼働させ、登録不要のデモとして公開した。

- 到達性：IGW / public subnet / EIP を追加し、EC2（IMDSv2・public subnet）で nginx（React 配信＋`/api`→Spring Boot）を公開。RDS は private のまま。
- 配備：GitHub Actions（build→S3 の不変成果物→SSM Run Command）で jar / dist を配備。配備は人の承認ゲート付き。
- TLS：Let's Encrypt（無料サブドメイン）で https 化。証明書は自動更新。
- 有効化した安全レール：Budgets（$5）／ RDS の prevent_destroy・deletion_protection ／ 人の承認 apply（`-auto-approve` は使わない）／ OIDC の鍵レス CI ／ 最小権限の instance role。
- 無料枠：EC2 t3.micro・RDS db.t3.micro・public IPv4 はいずれも 12 か月無料枠の範囲。
- 次に注意すべきこと：無料枠期限（12 か月）到来後の課金、証明書自動更新の監視、常時稼働のコスト。apply で無料枠プラン特有の制約（バックアップ保持上限・EBS 最小サイズ）に当たったため、無料枠プランの制約は事前に確認する。
