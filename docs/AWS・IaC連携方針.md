...
```
        ...
        IaCChangeProvider   MonitoringStatusProvider  InstanceStatusProvider
                │           │            │            │
   ┌────────────┴───┐  ┌────┴────┐  ┌────┴────┐  ┌────┴──────────┐
   │TerraformPlanAdapter│ │CloudFormation │ │CloudWatch │ │SystemsManager │   ← 本番Adapter（aws プロファイルで実装済み）
...
| `InstanceStatusProvider` | EC2/SSM 状態の取得 | `SsmStatusAdapter` | `MockInstanceStatusProvider` |

- 切替は Spring プロファイル（`local` は Mock、`aws` は実 Adapter）。`@Profile` / 設定で DI。
- 3 つの Port（IaC 差分／監視／インスタンス）は実装済み。既定は Mock、`aws` プロファイルで実 Adapter（S3 / CloudWatch / SSM・いずれも read-only）に切り替わる。
...
| 入力 | パーサ | 抽出方法 |
|---|---|---|
| Terraform plan（テキスト） | `TerraformDiffParser` | `# ... will be destroyed` / `-/+ ... replacement` 等の構文 |
| terraform plan -json（将来） | `TerraformJsonParser` | `resource_changes[].change.actions`（create/update/delete/replace）を直接マッピング |
| CloudFormation Change Set | `CloudFormationDiffParser` | `Action: Add/Modify/Remove/Replace`・ResourceType |
